...
const filtered = computed(() => {
  ...
  if (activeFilter.value === 'admins') list = list.filter(u => u.is_admin)
  else if (activeFilter.value === 'users') list = list.filter(u => !u.is_admin)
...
watch(itemsPerPage, (v) => {
  localStorage.setItem('users_items_per_page', v.toString())
  currentPage.value = 1
})
...
onMounted(() => {
  ...
  if (saved) {
    ...
    if (!isNaN(parsed) && [5, 10, 25, 50, 100].includes(parsed)) itemsPerPage.value = parsed
  }
  load()
})
</script>
...
.filter-tab { padding: 6px 12px; border-radius: 6px; font-size: 13px; font-weight: 600; color: var(--muted); transition: all 0.15s; }
.filter-tab:hover { color: var(--text); }
.filter-tab.active { background: var(--surface); color: var(--text); box-shadow: 0 2px 6px rgba(0,0,0,0.15); }
...
.add-user-btn:hover { opacity: 0.95; transform: translateY(-1px); }
...
.table td { padding: 12px 20px; border-top: 1px solid var(--border); vertical-align: middle; }
.user-row { cursor: pointer; transition: background-color 0.15s; }
...
.chip { display: inline-block; padding: 4px 10px; border-radius: 20px; font-size: 10.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }
...
@media (max-width: 768px) { .pagination-container { flex-direction: column; align-items: stretch; text-align: center; } }
.pagination-info { font-size: 13px; color: var(--muted); font-weight: 500; }
.pagination-info strong { color: var(--text); font-weight: 700; }
.pagination-controls { display: flex; align-items: center; flex-wrap: wrap; gap: 20px; }
@media (max-width: 768px) { .pagination-controls { flex-direction: column; align-items: center; } }
...
.modal-form label { font-size: 11px; font-weight: 700; color: var(--muted); text-transform: uppercase; }
.modal-form input {
  background: var(--surface2); border: 1.5px solid var(--border);
  border-radius: 10px; padding: 10px 14px; color: var(--text); transition: border-color 0.15s;
}
.modal-form input:focus { border-color: var(--accent); outline: none; }
.error-msg { color: var(--danger); background: var(--danger-dim); border-radius: 8px; padding: 10px; font-size: 12.5px; font-weight: 600; }
.modal-buttons { display: flex; justify-content: flex-end; gap: 10px; margin-top: 8px; }
.modal-buttons .btn { padding: 10px 16px; border-radius: 10px; font-weight: 700; font-size: 13px; transition: opacity 0.15s; }
